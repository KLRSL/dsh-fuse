...
console.log('\n[1] dsh-fuse 围栏渲染（登录页）')
{
  ...
  // 宽度约束：与输入框对齐（--dsh-chat-content-width 748px 兜底），全屏不撑满
  const holderStyle = document.getElementById('fuse-style') || [...document.querySelectorAll('style')].find((s) => s.textContent.includes('fuse-root-holder'))
  check('宽度约束样式存在（max-width 与输入框对齐）', !!holderStyle && holderStyle.textContent.includes('fuse-root-holder') && holderStyle.textContent.includes('max-width'), holderStyle ? 'style found' : 'no style')
