...
console.log('\n[4] 点击打开 UI 工作台面板')
{
  ...
  check('示例页面标题（欢迎回来·登录页）', !!title && title.textContent.includes('欢迎回来'), title && title.textContent)
  const inputs = [...document.querySelectorAll('.fuse-panel-body .fuse-input')]
  check('登录页输入框（用户名/密码）', inputs.length === 2, String(inputs.length))
  const loginBtn = document.querySelector('.fuse-panel-body .fuse-btn')
  check('主按钮「登 录」', !!loginBtn && loginBtn.textContent.includes('登 录'), loginBtn && loginBtn.textContent)
