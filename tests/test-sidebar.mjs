...
console.log('\n[4] 点击打开 UI 工作台面板')
{
  ...
  check('画板标题（记忆工作台设计稿）', !!title && title.textContent.includes('记忆工作台'), title && title.textContent)
  const stats = [...document.querySelectorAll('.fuse-panel-body .fuse-stat .v')].map((s) => s.textContent)
  check('画板状态卡（151/15/512维/26）', stats.includes('151') && stats.includes('15') && stats.includes('512维') && stats.includes('26'), stats.join(','))
  const h3s = [...document.querySelectorAll('.fuse-panel-body h3')].map((s) => s.textContent)
  check('画板分区（记忆构成/记忆流）', h3s.some((s) => s.includes('记忆构成')) && h3s.some((s) => s.includes('记忆流')), h3s.join(','))
