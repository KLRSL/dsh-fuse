...
.fuse-unsent{color:var(--fs-shell-error,#C62828);font-size:var(--fs-fs-caption);margin-right:auto}
...
.fuse-label{display:block;font-size:12px;font-weight:500;color:var(--fuse-muted,#69707E);margin-bottom:5px}
...
.fuse-stat .l{font-size:12px;color:var(--fuse-muted,#69707E)}
.fuse-table{width:100%;border-collapse:collapse;font-size:13px}
.fuse-table th{text-align:left;padding:9px 12px;border-bottom:2px solid var(--fuse-border,#E5E7EB);color:var(--fuse-muted,#69707E);font-weight:600;font-size:12px}
...
.fuse-list .d{font-size:12.5px;color:var(--fuse-muted,#69707E)}
...
.fuse-nav a{color:var(--fuse-muted,#69707E);text-decoration:none;cursor:pointer;padding-bottom:10px;border-bottom:2px solid transparent}
...
.fuse-tabs .tab{padding:8px 2px;cursor:pointer;color:var(--fuse-muted,#69707E);border-bottom:2px solid transparent}
...
.fuse-steps .dot.pending{background:var(--fuse-border,#E5E7EB);color:var(--fuse-muted,#69707E)}
...
.fuse-hero p{font-size:16px;color:var(--fuse-muted,#69707E);margin:0 0 24px}
.fuse-hero .actions{display:flex;gap:12px;justify-content:center}
.fuse-root-actions{display:flex;gap:12px;justify-content:flex-end;margin-top:16px;padding-top:12px;border-top:1px solid var(--fs-border,#E5E7EB)}
.fuse-btn-triggered{outline:2px solid var(--fs-primary,#2563EB);outline-offset:2px}
...
.fuse-chart .bar span{position:absolute;bottom:calc(100% + 4px);left:50%;transform:translateX(-50%);font-size:11px;color:var(--fuse-muted,#69707E)}
/* v1.2.5（自审 F3）：donut 与 line 的真渲染所需样式（此前它们被渲染成 badge 串） */
.fuse-donut{display:flex;align-items:center;gap:16px;padding:8px 0}
.fuse-donut .ring{width:120px;height:120px;border-radius:50%;flex:0 0 auto;background:conic-gradient(var(--fuse-primary,#2563EB) 0% 100%);position:relative}
.fuse-donut .ring::after{content:"";position:absolute;inset:26px;border-radius:50%;background:var(--fuse-bg,#FFFFFF)}
.fuse-donut .legend{display:flex;flex-direction:column;gap:6px;font-size:12px;color:var(--fuse-text,#1A1A1A)}
.fuse-donut .legend .item{display:flex;align-items:center;gap:6px}
.fuse-donut .legend .dot{width:10px;height:10px;border-radius:3px;flex:0 0 auto}
.fuse-linewrap{display:flex;flex-direction:column;gap:4px;padding:8px 0}
.fuse-line{width:100%;height:120px;display:block}
.fuse-linewrap .labels{display:flex;justify-content:space-between;font-size:11px;color:var(--fuse-muted,#69707E)}
...
`
    ...
    function loadThemes() {
      ...
      if (!themeLoad) {
        themeLoad = fetch(CONFIG_API, { headers: { accept: 'application/json' } })
          ...
          // v1.2.5（自审 F9）：失败也不缓存——旧实现把失败的 promise 一直挂在 themeLoad 上，
          // 于是 🔄 刷新按钮再调 loadThemes() 只会拿到同一个失败结果，令牌永远补不上。
          .finally(() => { themeLoad = null })
      }
      return themeLoad
    }
    ...
    function renderNode(node, ctx, key) {
      ...
      switch (type) {
        // ---- 展示 ----
        case 'text': {
          // v1.2.5（自审 F6）：字号此前是硬编码，而且与设计令牌阶梯不一致——
          // 令牌阶梯是 12/14/16/20/28/36/48（没有 24），hero 的 h1 用 36，
          // 于是同一个"h1"在系统里有 28 与 36 两种尺寸，text 的 h2=24 更是阶梯外的值。
          // 现在统一走令牌变量（发渲染产物令牌，随围栏主题覆盖到卡片根）。
          const sizes = {
            h1: 'var(--fs-fs-h1,36px)',
            h2: 'var(--fs-fs-h2,28px)',
            h3: 'var(--fs-fs-h3,20px)',
            body: 'var(--fs-fs-body,14px)',
            bodyLg: 'var(--fs-fs-bodylg,16px)',
            caption: 'var(--fs-fs-caption,12px)',
            muted: 'var(--fs-fs-caption,12px)',
          }
        ...
        case 'chart': {
          ...
          if (kind === 'bars') {
            ...
            return [box, true]
                    ...
                    if (kind === 'donut') {
                      // v1.2.5（自审 F3）：donut 此前与 line 一样被渲染成一串 badge —— 模型以为画了图，实际只是文字。
                      // 现在用 conic-gradient 画真环 + 图例（纯 DOM，无 innerHTML）。
                      const box = el('div', 'fuse-donut')
                      const ring = el('div', 'ring')
                      const total = data.reduce((s, d) => s + (Number(d.value) || 0), 0) || 1
                      const palette = ['var(--fuse-primary,#2563EB)', 'var(--fuse-accent,#049BDF)', 'var(--fuse-success,#2E7D32)', 'var(--fuse-warning,#ED6C02)', 'var(--fuse-error,#C62828)']
                      let acc = 0
                      const stops = []
                      data.forEach((d, i) => {
                        const pct = Math.max(0, (Number(d.value) || 0) / total * 100)
                      ...
                      data.forEach((d, i) => {
                        ...
                        dot.style.background = d.color ?? palette[i % palette.length]
                        item.append(dot, el('span', null, `${d.label ?? ''} ${d.value ?? ''}`))
                        legend.appendChild(item)
                      })
                      box.appendChild(legend)
                      return [box, true]
          }
          // v1.2.5（自审 F3）：line 此前也退化成 badge 串；现在画真折线（SVG polyline，不执行任何模型文本）
          const pts = data.map((d) => Number(d.value) || 0)
          if (pts.length >= 2) {
            const W = 320
            const H = 120
            ...
            svg.setAttribute('class', 'fuse-line')
            const poly = document.createElementNS(NS, 'polyline')
            poly.setAttribute('points', pts.map((v, i) => `${(i / (pts.length - 1) * W).toFixed(1)},${(H - Math.max(4, v / top * (H - 12))).toFixed(1)}`).join(' '))
            poly.setAttribute('fill', 'none')
            poly.setAttribute('stroke', 'var(--fuse-primary,#2563EB)')
            poly.setAttribute('stroke-width', '2')
            svg.appendChild(poly)
            const wrap = el('div', 'fuse-linewrap')
            wrap.appendChild(svg)
            const labels = el('div', 'labels')
            data.forEach((d, i) => {
              if (i === 0 || i === data.length - 1 || data.length <= 8) labels.appendChild(el('span', null, String(d.label ?? '')))
            })
        ...
        case 'button': {
          // v1.2.4：兼容规格用词——tone（primary|ghost）与 label（按钮文案），旧字段 style/text 继续支持。
          const b = el('button', 'fuse-btn ' + String(node.style ?? node.tone ?? 'secondary') + (node.full ? ' full' : '') + (node.small ? ' small' : ''))
          b.textContent = String(node.text ?? node.label ?? '')
          if (node.action) {
            b.addEventListener('click', (event) => {
              // 阻止冒泡：否则同一次点击会被外层容器再当成「走查点击」发出 [fuse-inspect]（自审 F7）
    ...
    /** v1.2.5（自审 F1）：同一个容器只挂一次走查监听——现在 DOM 与 registry 两条通道都会调它 */
    const inspectedContainers = new WeakSet()

    function installInspector(container, ctx, fenceKey) {
      if (!container || inspectedContainers.has(container)) return
      inspectedContainers.add(container)
    ...
    // v1.2.5（自审 F8）：回传失败此前只 console.warn —— 用户点了元素却什么都没发生、也无从解释。
    // 现在把"没回传成功"写到卡片工具栏上（6 秒后自动消失），控制台日志保留作诊断用。
    let lastCard = null
    function warnOnCard(text) {
      try {
        if (!lastCard || !lastCard.isConnected) return
        const bar = lastCard.querySelector('.fuse-toolbar')
        if (!bar || bar.querySelector('.fuse-unsent')) return
        const hint = document.createElement('span')
        hint.className = 'fuse-unsent'
        hint.textContent = text
        bar.prepend(hint)
        setTimeout(() => { try { hint.remove() } catch { /* 忽略 */ } }, 6000)
      } catch { /* 提示本身不得影响渲染 */ }
    }
    ...
    function sendInspect(ctx, fenceKey, elementKey, styles) {
      ...
      if (conversation?.send) {
        conversation.send(msg).catch(() => { warnOnCard('走查数据未回传：会话已结束') })
      } else {
        warnOnCard('走查数据未回传：当前没有会话通道')
    ...
    function sendAction(ctx, elementKey, action, payload) {
      ...
      if (conversation?.send) {
        conversation.send(msg).catch(() => { warnOnCard('动作未回传：会话已结束') })
      } else {
        warnOnCard('动作未回传：当前没有会话通道')
    ...
    function renderFenceCard(ctx, fenceKey, raw, onRerender) {
      ...
      // v1.2.4：根级 actions —— 系统指令与文档都要求模型把**主操作**写在这里，
      // 而渲染器此前没有任何读取点，导致「主操作静默消失」（自审 F4）。现在渲染成底部操作条。
      const rootActions = Array.isArray(spec.actions) ? spec.actions : []
      if (rootActions.length > 0) {
        const bar = el('div', 'fuse-root-actions')
        rootActions.forEach((a, i) => {
          const [btn] = renderNode({ ...a, type: 'button' }, ctx, `${fenceKey}:act:${i}`)
          btn.dataset.fuseKey = `${fenceKey}:act:${i}`
          bar.appendChild(btn)
        })
      ...
      lastCard = card // v1.2.5（自审 F8）：供 warnOnCard 定位"最近一张预览卡"
      ...
      if (typeof registerFn === 'function') {
        try {
          disposers.push(registerFn(FENCE_LANG, (raw, key, context) => {
            ...
            // v1.2.5（自审 F1）：此前这里只返回卡片，没挂走查器 —— 于是「点击预览元素取
            // getComputedStyle 回传」的像素级走查在契约宿主（registry 通道）下静默失效，
            // 而 README 把走查列为三大卖点之一。这里补挂（幂等，由 installInspector 内部守卫）。
            const built = renderFenceCard(ctx, `fence:${String(key)}`, raw, () => {})
            try { installInspector(built.card, ctx, `fence:${String(key)}`) } catch { /* 走查是增强，不阻断渲染 */ }
            return built.card
          }))
